...
uses Unit1, Unit5, Thread, FMX.Forms;
{$R *.dfm}
...
begin
  ...
  FDQuery2.SQL.Text :=
    Format('insert into %s("PAGE",image,sub) values(:page_id,:image,:sub);',
    [fn]);
  FDQuery2.Params.ArraySize := Form5.ListBox1.Count;
  FDTable1.Close;
  FDTable1.Open(fn);
  Form1.ProgressBar1.Max := Form5.ListBox1.Count;
  paraProc;
